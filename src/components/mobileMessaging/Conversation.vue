  ...
  <div>
    <div class="main-div h-auto app-text-color-secondary rounded-lg">
      <div class="md:grid md:grid-cols-9">
        <!-- conversation list -->
        <div
          class="bg-[#f2f2f2] left-rounded md:col-span-4"
        ...
        <!-- Conversation -->
        <div class="md:col-span-5" :style="getBackgroundImage()">
          <div v-if="show_messaging_list" class="bg-[#fff] lg:rounded-lg">
...
export default {
  ...
  setup(props, context) {
    ...
    function setControlsVisibilityAsPerDevice() {
      ...
      if (
        route.name == appConstants.routes.MOBILEMESSAGING ||
        route.name == appConstants.routes.MOBILENEW_CONVERSATION
      ) {
      ...
      if (
        route.name == appConstants.routes.MOBILECONVERSATION ||
        route.name == appConstants.routes.MOBILENEW_CONVERSATION
      ) {
        // let footer = document.querySelector(".footer");
