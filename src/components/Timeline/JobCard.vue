<template>
  <div class="grid grid-cols-1">
    <div v-for="(jobs, index) in jobs" :key="index">
      <div
        :class="getBackgroundColor()"
        class="rounded-[8px] lg:p-2 md:p-2 px-1 py-2 app-bg-color-gradient cursor-pointer mb-3 app-bg-color-secondary hover:app-bg-hover-gradient app-bg-hover-gradient"
      >
        <div class="job-card-container">
          ...
          <div
            class="lg:ml-2 md:ml-2 ml-1 w-full"
            @click="goToJobDetails(jobs)"
          >
              ...
              >
                ...
                <p class="text-[#959595] font-normal text-xs italic">
                  ...
                  <span v-if="jobs.job_application.created_at"
                    >on
                    {{ setAppliedDate(jobs.job_application.created_at) }}</span
                  >
                </p>
              </div>
              <div v-else>
          ...
          <div
            class="flex w-full flex-col justify-between items-end cursor-auto"
          >
            ...
            <div v-if="jobs.bookmark_id?.id" class="cursor-pointer">
              <img src="/src/assets/img/Bookmark-Fill.svg" alt="" />
            </div>
...
export default {
  ...
  setup(props) {
    ...
    function getBackgroundColor() {
      if (route.name == appConstants.routes.JOBS) {
        return "app-border-static hover:app-border-hover app-bg-hover";
      } else {
...
.job-card-container {
  display: grid;
  grid-template-columns: 66px auto 120px;
}
...
/* @media (max-width: 320px) {
  .job-card-container {
    display: grid;
    grid-template-columns: 32px 160px 48px;
  }
} */

@media only screen and (max-width: 768px) {
  .job-card-container {
    grid-template-columns: 38px 210px 38fr;
  }
}
</style>
