  ...
  <div class="cls-conversationCard">
    <div class="py-[8px]">
      <div class="flex flex-row items-center">
        ...
        >
          <h1
            class="text-base font-size-16 font-weight-600 app-text-color-primary overflow-hidden whitespace-nowrap text-ellipsis show-ellipsis"
          >
            {{ conversation.name }}
          </h1>
        ...
        <div class="flex flex-col items-end flex-shrink-0">
          ...
          <!-- <span
            v-if="setVisibilityOfAcceptButton()"
            @click="acceptConversation()"
            class="rounded-2xl bg-[#3a3a41] text-white text-xs h-6 p-2 flex items-center cursor-pointer"
            >Accept</span
          > -->
          <!-- <div
            v-if="getUnreadMessageCount() > 0"
            class="rounded-full bg-[#3a3a41] text-white text-xs py-1 px-2 flex items-center"
          >
            <span class="">{{ getUnreadMessageCount() }}</span>
            <Badge :value="getUnreadMessageCount()" class="mr-2"></Badge>
          </div> -->

          <!-- <div v-if="getUnreadMessageCount() > 0">
            <Badge
              :value="getUnreadMessageCount()"
              class="mr-2 gray-badge"
            ></Badge>
          </div> -->
          <div
            v-if="getUnreadMessageCount() > 0"
...
/* .cls-conversationCard {
  background-color: var(--app-secondary-color);
} */
.cls-conversationCard:hover {
  background-color: var(--app-gradient-first-color);
  /* background-color: rgb(from var(--app-primary-color) r g b / 0.5); */
  /* filter: brightness(var(--lighter-primary)); */
  /* background-color: calc(var(--app-primary-color) + var(--lighter-primary)); */
}
</style>
