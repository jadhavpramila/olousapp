<template>
  <div class="cls-joblist form-container">
    ...
    <div class="grid md:grid-cols-4 grid-cols-12 gap-3">
      <div class="md:col-span-4 col-span-12">
        <div class="grid grid-cols-6 gap-3">
          <div
            class="w-full rounded-md col-span-2 max-h-[40rem] lg:block md:block hidden"
          >
...
export default {
  ...
  async setup({ emit }) {
    ...
    let selectedJobFilterType = ref(appConstants.jobType.ALL);
    ...
    onBeforeMount(() => {
      ...
      if (isUserLoggedIn.value) {
        if (route.query.isFromBookMark) {
          ...
          // load();
          loadJobList(store.getters["jobs/getter_currentJobType"], "");
        }
      } else {
        const url = `/${appConstants.routes.JOBS}`;
        window.history.pushState({}, "", url);
        ...
        if (isUserLoggedIn.value) {
          await store
            ...
            .then((res) => {
              ...
              jobList_scroll_id = res.scroll_id;
              isDataFetching.value = false;
              getMoreJobs();
              getRecommendedJobs();
            });
    ...
    function filterAllJob(eventData) {
      ...
      loadJobList(isFilterEvent, advancedFilterData);
    }

    function loadJobList(job_type, advancedFilterData) {
      selectedJobFilterType.value = job_type;
      if (job_type == jobType.ALL) {
        //console.log("if all ");
        let obj = {
          page: 1,
          jobsData: null,
        ...
        jobList_scroll_id = "";
        load();
      } else if (job_type == jobType.APPLIED) {
        isDataFetching.value = true;
        ...
        clickApplied();
      } else if (job_type == jobType.BOOKMARKED) {
        //console.log("in if previous")
        let obj = {
          page: 1,
          jobsData: null,
        ...
        clickBookmark();
      } else if (job_type == jobType.ADVANCED) {
        isDataFetching.value = true;
        isNextCall.value = true;
        args.page = 1;
        args.limit = 20;
